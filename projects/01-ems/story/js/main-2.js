...
// Luis: Cargo estilos después de cargar los datos en lugar de antes
...
// Borrar los anteriores
  ...
  if (map.hasLayer(dt2)) {
    map.removeLayer(dt2) //Luis: Quitar palabra reservada "return"
  }
  if (map.hasLayer(dt3)) {
    map.removeLayer(dt3)  //Luis: Quitar palabra reservada "return"
  }

  color2 = "dark"
  
  dt2 = omnivore.topojson('data/card.json', null, customLayer).addTo(map)

  //Luis: customLayer ya declarado de manera global (en linea 158). Por eso, quitamos de aquí la palabra reservada "var"
...
$('#respir').click(function(){
  ...
  dt2 = omnivore.topojson('data/respir.json', null, customLayer).addTo(map)
